...
function get_distmx(distmxs::Vector{Tuple{Float64,Array{Any,1}}},time::Float64)
    for (k,disttuple) in enumerate(distmxs)
        time1 = disttuple[1]
        if time < time1
            return distmxs[k-1][1]
        end
    end
    return distmxs[end][1]

function LightGraphs.dijkstra_shortest_paths(g::AbstractGraph,
    srcs::Vector{U},
    distmxs::Vector{Tuple{Float64,Array{Any,1}}}; #First index is time, next is distmx for all times after this time
    allpaths=false,
    trackvertices=false
